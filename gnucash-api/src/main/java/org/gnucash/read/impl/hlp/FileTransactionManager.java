...
public class FileTransactionManager {
	...
	for ( GnucashTransaction trx : trxMap.values() ) {
	    try {
		// Altern. 1: Geht nicht, weder read noch write
//		for ( GnucashTransactionSplit splt : getTransactionSplits_readAfresh(trx.getId()) ) {
//		    trxSpltMap.put(splt.getId(), splt);
//		}
		// Altern. 2: geht read und write ( acct OK, trx NOT OK )
